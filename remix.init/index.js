...
const main = async ({ isTypeScript, packageManager, rootDirectory }) => {
  ...
  const DEPLOY_ARM_WORKFLOW_PATH = path.join(
    rootDirectory,
    '.github',
    'workflows',
    'deploy-arm.yml'
  ...
  const [
    ...
    fs.readFile(DEPLOY_ARM_WORKFLOW_PATH, 'utf-8'),
  ...
  const newEnv = env
    ...
    .replace(/^PUBLIC_APP_NAME=.*$/m, `PUBLIC_APP_NAME="${APP_NAME}"`)
  ...
  const matches = newReadme.match(new RegExp('pnpm migrate .*'))
  if (matches.length) {
    const newMigration = matches[0].replaceAll('-', '')
    newReadme = newReadme.replace(matches[0], newMigration)
  }
  ...
  const newDeployArmWorkflow = deployArmWorkflow.replace(
    new RegExp(escapeRegExp(REPLACER), 'g'),
    APP_NAME
  )
  ...
  const fileOperationPromises = [
    ...
    fs.writeFile(DEPLOY_ARM_WORKFLOW_PATH, newDeployArmWorkflow),
    packageJson.save(),
    fs.copyFile(
  ...
  if (!isTypeScript) {
    const parsedDeployArmWorkflow = YAML.parse(newDeployArmWorkflow)

    fileOperationPromises.push(
      ...cleanupDeployWorkflow(
        parsedDeployArmWorkflow,
        DEPLOY_ARM_WORKFLOW_PATH
      )
    )
  }
