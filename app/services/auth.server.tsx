  ...
  new OTPStrategy(
    {
      ...
      storeCode: async (code) => {
        await db.otp.createOtp({ code })
      },
      sendCode: async ({ email, code, magicLink, user, form, request }) => {
        const url = new URL(request.url)
        const html = emailConstructors.login(
      ...
      validateCode: async (code) => {
        const [oneTimePasscode] = await db.otp.findOtpByCode(code)
      ...
      invalidateCode: async (code, active, attempts) => {
        await db.otp.updateOtp(code, active, attempts)
      },
    },
    async ({ email, code, form, magicLink, request }) => {
      ...
      if (!existingUser) {
        const [newUser, newOrganisation] = await db.transaction(
          async (store) => {
            const [newOrganisation] =
              await store.organisation.createOrganisation({
                name: 'Personal',
